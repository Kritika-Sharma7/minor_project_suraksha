...
import { LineChart, Line, XAxis, YAxis, Tooltip as RechartsTooltip, ResponsiveContainer, Area, AreaChart, PieChart, Pie, Cell } from 'recharts'
import { Activity, BarChart2, TrendingUp, ShieldCheck, Zap, BrainCircuit, Globe } from 'lucide-react'
import { useSafetyStore } from './safetyStore'
import { motion } from 'framer-motion'

export default function AnalyticsView() {
  const { history, chartData } = useSafetyStore()
  ...
  const avgRisk = history.length > 0 ? (history.reduce((a, b) => a + b.combinedScore, 0) / history.length).toFixed(1) : 0
  const sessionIntegrity = "99.8%"
  const meanJitter = "0.42"
    ...
    <div className="max-w-[1400px] mx-auto space-y-12 pb-24">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
         {[
           ...
           { label: 'MEAN JITTER', value: meanJitter, icon: TrendingUp, color: 'secondary' },
           { label: 'SESSION UPTIME', value: sessionIntegrity, icon: Zap, color: 'blue' },
           { label: 'GEO COVERAGE', value: '7.2km²', icon: Globe, color: 'cyan' }
         ].map(m => (
                 ...
                 <PieChart>
                   ...
                   >
                     {pieData.map((entry, index) => (
                       <Cell key={`cell-${index}`} fill={`url(#grad-${index})`} style={{ filter: `drop-shadow(0 0 10px ${entry.color}40)` }} />
                     ))}
                   </Pie>
                 </PieChart>
               </ResponsiveContainer>
          ...
          <div className="grid grid-cols-2 gap-4 w-full mt-10">
            {pieData.map(d => (
              <div key={d.name} className="flex flex-col p-4 rounded-2xl bg-white/[0.02] border border-white/[0.05]">
                 ...
                 <span className="text-lg font-display font-bold text-white/90">{((d.value/total)*100).toFixed(0)}%</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="xl:col-span-2 glass rounded-[2.5rem] p-10 border border-white/5 flex flex-col">
          <div className="flex items-center justify-between mb-10">
             <div>
               ...
               <p className="text-[10px] font-mono text-slate-500 mt-1 uppercase tracking-widest">Multi-Series session analysis</p>
             </div>
          ...
          <div className="flex-1 min-h-[400px]">
            {chartData.length > 0 ? (
               <ResponsiveContainer width="100%" height="100%">
                 <AreaChart data={chartData} margin={{ left: -30, right: 0, top: 10, bottom: 0 }}>
                    <defs>
                   ...
                   <RechartsTooltip cursor={{ stroke: 'rgba(255,255,255,0.05)' }} contentStyle={{ backgroundColor: 'rgba(10,14,23,0.9)', borderColor: 'rgba(255,255,255,0.1)', borderRadius: '16px', border: '1px solid rgba(255,255,255,0.05)', backdropFilter: 'blur(10px)' }} />
                   <Area type="monotone" dataKey="score" stroke="#4edea3" strokeWidth={3} fillOpacity={1} fill="url(#scoreG)" />
                   <Area type="monotone" dataKey="avg" stroke="#6366f1" strokeWidth={2} strokeDasharray="4 4" fill="none" />
                 </AreaChart>
               </ResponsiveContainer>
            ) : (
               <div className="h-full flex items-center justify-center">
                  <span className="text-[10px] font-mono text-slate-700 font-black uppercase lg:tracking-[1em] animate-pulse">Syncing Session Intelligence Matrix...</span>
               </div>
            )}
          </div>
        </div>
